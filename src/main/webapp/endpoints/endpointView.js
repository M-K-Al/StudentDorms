...
$(function () {
    ...
    $(".range").slider({
        range: "min", min: 0, max: 100, step: 0.1, slide: function (event, ui) {
            $(this).find(".ui-slider-handle").children().text(Math.round(ui.value));
        }
    }).each(function () {
        const dis = $(this);
        const value = dis.attr("value");
        dis.slider("value", value);
        dis.find(".ui-slider-handle").append(`<span class="hidden rounded-md px-4 py-2 text-center text-sm slider-popup">${value}</span>`);
    });

    $(".ui-slider-handle")
        .on("mouseenter", function () {
            const dis = $(this);
            if (!dis.hasClass("ui-state-active")) dis.children().removeClass("hidden");
